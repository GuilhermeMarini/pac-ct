...
- Modify: the local web-check checklist (eight mounts become nine)
...
- [ ] **Step 9: Tell the web-check checklist about the ninth mount**

In the local web-check checklist, section "2. Walk the eight mounts": change the heading to "nine mounts" and add `/gle-tabs/` to the list of prefixes. Check the rest of that file for other places the eight is spelled out and update them too.
...
Run the project's full check. All four must pass: `ruff`, `mypy` (no arguments), `pytest tests/ -q`, `python3 tools/build_dist.py --no-vendor`.
