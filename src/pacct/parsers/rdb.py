...
`pacct.parsers.rdb_cache`), e nao mais num diretorio por ferramenta chaveado
...
from pacct.paths import RDB_CACHE_DIR
...
def process_upload_stream(source, length: int, filename: str,
    ...
    base = Path(cache_root) if cache_root is not None else RDB_CACHE_DIR
