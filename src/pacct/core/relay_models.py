...
from pacct.paths import RELAY_MODELS_DIR as _MODELS_DIR
...
def load_relay_models(models_dir: Path | None = None,
    ...
    """Carrega (e cacheia) todos os JSONs em `data/relay_models/`."""
    ...
    if base.is_dir():
        for path in sorted(base.glob("*.json")):
            ...
            if m is None:
                continue
            cache[_normalize(m.model)] = m
            for a in (m.model, *m.model_aliases):
                alias[_normalize(a)] = m
    _CACHE = cache
    _ALIAS_INDEX = alias
    _LOADED = True
    return cache
