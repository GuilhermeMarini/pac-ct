...
def _cast_bytearray(data: AnyStr, debug: bool = True):
    ...
    # Confia no comprimento declarado no proprio frame (byte 2) ao inves de
    # splitar em b'=' -- esse byte (0x3D) pode aparecer dentro do payload
    # binario e truncava o frame indevidamente.
    if len(byte_array) >= 3:
        declared_len = byte_array[2]
        if declared_len > 0 and len(byte_array) >= declared_len:
            byte_array = byte_array[:declared_len]
...
def relay_dna_block(data: AnyStr, encoding: str = '', byteorder: str = 'big',
    ...
    # NOTA: nao removemos mais via RE_DNA_CONTROL (regex `\>?.*DNA`) -- ele eh
    # ganancioso e casa ate o ULTIMO "DNA" da resposta, o que descarta linhas
    # com nomes tipo "DNAUX1", "DNAUX2", etc. (visto no SEL-751). O filtro
    # `if ',' in line` abaixo ja descarta a linha de eco do comando ("DNA\r\n",
    # ">DNA\r\n") e qualquer prompt residual, sem o risco do regex.
...
def fast_meter_block(data: AnyStr, definition: dict, dna_def: List[List[str]],
    ...
    try:
        ...
        # Tolerant: numdigitalbank (from A5C1) e len(dna_def) (do DNA ASCII) podem
        # divergir em alguns reles -- ex.: SEL-751 reporta numdigitalbank=181 mas
        # DNA so devolve as banks visiveis no nivel de acesso atual / filtradas.
        # Itera o minimo dos dois para evitar IndexError e nao perder bits.
        n_banks = min(definition['numdigitalbank'], len(dna_def))
        if (definition['numdigitalbank'] != len(dna_def)
                and not getattr(fast_meter_block, '_warned_mismatch', False)):
            # Apenas avisa uma vez por processo. Nao raise: a aplicacao decide
            # se quer abortar via DnaDigitalsMisMatch passando strict=True
            # quando suportado, ou apenas seguir com os bits nomeados.
            print(
                f"[selprotopy] aviso: numdigitalbank={definition['numdigitalbank']} "
                f"!= len(dna_def)={len(dna_def)}; usando {n_banks} banks."
            )
            fast_meter_block._warned_mismatch = True
        for target_row_index in range(n_banks):
        ...
        # Remove the '*' Key (placeholder de bit nao usado)
        struct['digitals'].pop('*', None)
